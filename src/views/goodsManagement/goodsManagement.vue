<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey'
      ...
      <el-table-column align="center"
                       label="性质"
                       prop="categore_name"></el-table-column>
      <el-table-column align="center"
                       label="隶属部门">
        <template scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]"
      ...
      <el-form class="small-space"
        ...
        <el-form-item label="上传图片"
          ...
          <el-upload class="avatar-uploader"
            ...
            <img v-if="temp.img_url"
                 :src="temp.img_url"
                 class="avatar">
            <i class="el-icon-plus avatar-uploader-icon"
               v-else></i>
            <div slot="tip"
                 class="el-upload__tip">只能上传jpg、png文件，且不超过500KB，推荐尺寸750x560px</div>
          </el-upload>
        </el-form-item>
        ...
        <el-form-item label="隶属部门"
          ...
          <el-select class="filter-item"
                     ...
                     placeholder="请选择">
            <el-option v-for="(item,key) in enumValues.departments"
                       :key="key"
                       :label="item"
                       :value="key">
            </el-option>
        ...
        <el-form-item label="物品性质"
          ...
          <el-radio-group v-model="temp.categore">
            <el-radio v-for="(item,key) in enumValues.categores"
                      :key="key"
                      :label="key">
              {{item.name}}
            </el-radio>
        ...
        <el-row>
          <el-col :span="14">
            <el-form-item label="领用频率"
                          prop="frequency"
                          v-if="temp.need_return===0">
              <el-input-number v-model.number="temp.frequency"
                               :min="0"
                               :max="255"
                               style="display: inline-block; width: 60%;"></el-input-number>
              <span style="display: inline-block;line-height: 36px;vertical-align: top;">/人·月
                <span style="color:#ccc;font-size: 10px;">0为不限</span>
              </span>
            </el-form-item>
            <el-form-item label="可借时长"
                          v-else-if="temp.need_return===1">
              <el-input-number v-model.number="temp.duration"
                               :min="0"
                               :max="255"
                               style="display: inline-block; width: 60%;"></el-input-number>
              <span style="display: inline-block;line-height: 36px;vertical-align: top;">天
                <span style="color:#ccc;font-size: 10px;">0为不限</span>
              </span>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="10">
            <el-form-item label="库存"
                          prop="stock">
              <el-input-number v-model.number="temp.stock"
                               :min="0"
      ...
      <div slot="footer"
        ...
        <el-button v-else
                   type="primary"
                   @click="update('goodsDetailForm')">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
